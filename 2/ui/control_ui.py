...
class UIController:
    def __init__(self, board, search_algo, value_func):
        ...
        self.current_point = (self.board.length / 2, self.board.length / 2)
    ...
    def current_block(self):
        return self.board.users[player1].block_pool.block_list[self.current_choose]\
    ...
    def key_enter(self, event):
        if self.board.user_can_put_block_on_point(player1, self.current_block(), self.current_point):
            self.app.draw_block_on_point(self.current_block(), self.current_point, 'green', True)
            self.board.user_put_block_on_point(player1, self.current_block(), self.current_point)
    ...
    def key_space(self, event):
        self.current_transform = (self.current_transform + 1) % \
                                 len(self.board.users[player1].block_pool.block_list[self.current_choose].
    ...
    def key_tab(self, event):
        self.current_choose = (self.current_choose + 1) % len(self.board.users[1].block_pool.block_list)
        self.current_transform = 0
        while not self.current_block().in_board_range(self.board, self.current_point):
            if self.current_point.x > self.board.length / 2:
                self.current_point.x -= 1
    ...
    def if_game_ends(self):
        if len(self.board.user_possible_puts(player1)) == 0 and \
                        len(self.board.user_possible_puts(player2)) == 0:
            print 'Game Ends'
            score1 = 0
            score2 = 0
            for point in self.board.point_list:
                if self.board.point_inuse_dic[point] == player1:
                    score1 += 1
                elif self.board.point_inuse_dic[point] == player2:
                    score2 += 1
            print 'Score:', 'Player:', score1, 'Computer:', score2

    def computer_calculate(self):
        if len(self.board.user_possible_puts(player2)) == 0:
            print 'User 2 out of move'
            return
        move = self.search_algo(self.board, player2, self.value_func)
        self.app.draw_block_on_point(move[0], move[1], 'purple', True)
        self.board.user_put_block_on_point(player2, move[0], move[1])
